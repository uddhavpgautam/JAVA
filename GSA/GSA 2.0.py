...
def main ():
    ...
    #najprije dodajem pomakni, a onda i reduciraj određenim redosljedom tako da se izbjegnu nejednoznačnosti, kad analizator bude čitao
    #tablicu i npr. popuni tablicu na mjestu 0 a pomakni, neće više moći na to mjesto staviti 0 b reduciraj,
    #drugim riječima morati će se provjeriti ako je to mjesto u tablici već popunjeno.
    ...
    #ako unutar stanja(0,1,2,) imamo neko stanje kome je točka na krajnjem mjestu onda za to stanje(prvo na koje se naiđe) se dodaje u tablicu
    #akcija reduciraj to stanje za sve navedene znakove
    #da bi izbjegli nejednoznačnost čim naletimo na prvi reduciraj izlazimo iz petlje
    #ako ne nademo reduciraj jednostavno necemo nista zapisati u talicu akcija
    #te ce se racunati kao da je to mejsto prazno
    #mozda naides na prihvati, u tom slučaju to ispisi
    for kljuc in dictDKA:
        for stanje in dictDKA[kljuc]:
            #pogledaj je li točka na kraju
            ind = stanje.find('#')
            if stanje[ind+1] == '{':
                #na kraju je
                #pogledaj za koje znakove
                #se izvrsava akcija reduciraj
                lijevo = stanje[:ind+1]
                desno = stanje[ind+2:]
                desno = list(desno)

                #pogledaj je li prihvati, je li lijevo od strelice
                #novi početni nezavrsni znak
                poc = lijevo.split('->')
                if poc [0] == '<NoviNezZnak>':
                    ispis = str(kljuc)+' % Prihvati '+str(1)
                    TablicaAkcija.write(ispis)
                    TablicaAkcija.write('\n')

                del desno[-1]
                if isinstance(desno,str):
                    ispis = str(kljuc)+' '+desno+' Reduciraj '+lijevo
                    TablicaAkcija.write(ispis)
                    TablicaAkcija.write('\n')
                else:
                    #lista je
                    for znak in desno:
                        ispis = str(kljuc)+' '+znak+' Reduciraj '+lijevo
                        TablicaAkcija.write(ispis)
                        TablicaAkcija.write('\n')
    ...
    #napravi tablicu novo stanje
    for kljuc in dictDKA:
        for znak in nezavrsni:
            for stanje in dictDKA[kljuc]:
                tmp = stanje+','+znak
                if tmp in dictPrijelazi:
                    #ukoliko nađeš prijelaz pogledaj gdje ideš, ideš u samo jedno
                    ...
                    for key in dictDKA:
                        if odrediste in dictDKA[key]:
                            odredisteDKA = key
                            break
                    ispis = str(kljuc)+' '+znak+' Stavi '+str(odredisteDKA)
                    TablicaNovoStanje.write(ispis)
                    TablicaNovoStanje.write('\n')
                else:
                    pass
                    #inače odbaci, odnosno "ostavi to mejsto u tablici prazbno
                    #ne zapisuj ništa u datoteku
