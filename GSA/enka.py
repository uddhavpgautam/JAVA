...
class Enka(object):
    ...
    def nadiEps(self,stanje,prijelazi):
        #dobiva jedno stanje
        #nalazi sva stanja u koje se prelazi eps prijalzima iz tog stanja (samo prvu razinu)
        ...
        #ako je naso neke eps prijelaze
        if isinstance(stanja,list) and len(stanja) > 1:
            for stanje in stanja:
                primljeno = self.nadiEps(stanje,prijelazi)
                if primljeno[0] == '#' or primljeno[0] == '{':
                    continue
                else:
                    self.nadiEps(primljeno[0],prijelazi)
