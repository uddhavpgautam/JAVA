...
def izmedjuZagrada(red):
    tmp = list(red.split(">")[0])
    del tmp[0]
    tmp = ("").join(tmp)
    return tmp

def ostatakReda(red):
    tmp = list(red.split(">")[1])
    return tmp
...
def main():
    ...
    string = "luka"
    while(True):
        prvi_znak = list(dictionary1.ulazna_lista[pozicija])[0]
        # if prvi_znak == EOF: break
        stanje = izmedjuZagrada(dictionary1.ulazna_lista[pozicija])
        dictionary1.last_row += 1
        ostatak_reda = ostatakReda(dictionary1.ulazna_lista[pozicija])
        automat1 = Automat()
        ...
        # datoteka.write(poc)
        # datoteka.write(prih)
        if dictionary1.ulazna_lista[dictionary1.last_row] == "{": dictionary1.last_row += 1
        ...
        # datoteka.write('\n')
        continue                    #skoci na pocetak petlje
